...
function argumentAfter(text, command) {
    const raw = text.slice(command.length).trim();
    return raw.startsWith('+') ? raw.slice(1).trim() : raw;
}
function formatProjects(projects, selectedId) {
    if (projects.length === 0)
        return '还没有项目。使用 /np + 项目名 创建。';
    return projects.map((project, index) => {
        const selected = project.id === selectedId ? '（当前）' : '';
        return `${index + 1}，${project.name}${selected}`;
    }).join('\n');
}
function formatConversations(conversations, selectedId) {
    if (conversations.length === 0)
        return '当前项目还没有对话。使用 /nc 创建。';
...
function projectByArgument(projects, argument) {
    if (!/^\d+$/u.test(argument))
        return exactProject(projects, argument);
...
export class FeishuCommandController {
    bridge;
    selections;
    constructor(bridge, selections) {
        this.bridge = bridge;
        this.selections = selections;
    }
    async markConversationCompleted(chatId, conversationId) {
    ...
    async handle(chatId, input) {
        ...
        if (text.startsWith('/up')) {
            ...
            if (argument.length === 0)
                return { text: '用法：/up + 项目名或索引，例如 /up + 1' };
            const projects = await this.bridge.listProjects();
        ...
        if (text === '/lc') {
            if (selection.projectId === undefined)
                return { text: '请先使用 /up + 项目名或索引 进入项目。' };
        ...
        if (text.startsWith('/uc')) {
            ...
            if (rawIndex.length === 0) {
                if (selection.lastCompletedConversationId === undefined)
                    return { text: '还没有可进入的已完成对话。请使用 /uc + 索引进入对话。' };
                const conversation = await this.bridge.conversationInfo(selection.lastCompletedConversationId);
                if (conversation === undefined)
                    return { text: '最近完成的对话已不存在，请使用 /lc 查看当前项目对话。' };
            ...
            if (!Number.isSafeInteger(index) || index < 1)
                return { text: '用法：/uc + 索引，例如 /uc + 1' };
            const conversations = await this.bridge.listConversations(selection.projectId);
            const conversation = conversations[index - 1];
            if (conversation === undefined)
                return { text: `对话索引超出范围：${rawIndex}` };
            const next = { ...selection, conversationId: conversation.id };
            delete next.lastCompletedConversationId;
            await this.selections.set(chatId, next);
            const history = await this.bridge.recentMessages(conversation.id, 2);
        ...
        if (text.startsWith('/'))
            return { text: `未知指令：${text.split(/\s/u, 1)[0]}\n\n${HELP}` };
        if (selection.conversationId === undefined)
            return { text: '请先使用 /uc + 索引进入对话，或使用 /nc 创建对话。' };
