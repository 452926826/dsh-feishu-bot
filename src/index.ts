...
import { readBindingView, type FeishuBindingView } from './binding-status.js'
...
export function apply(ctx: Context): void {
  ...
  let websocket: FeishuBindingView['websocket'] = 'disconnected'
  const approvals = new ApprovalCoordinator(async (chatId, text) => {
    if (bot === undefined) throw new Error('Feishu bot is not ready')
  ...
  const start = async (): Promise<void> => {
    ...
    bot = new FeishuBot({
      ...
      onMessage: async message => (await controller.handle(message.chatId, message.text)).text,
      onReady: () => {
        websocket = 'connected'
  ...
  ctx.effect(() => {
    void start().catch(error => {
      if (!abort.signal.aborted) logger.error(error instanceof Error ? error.stack ?? error.message : String(error))
    })
    return async () => {
      abort.abort()
      await bot?.stop()
...
async function registerUntilComplete(options: Parameters<typeof registerByQrCode>[0]): Promise<FeishuCredentials> {
  let attempt = 0
  while (true) {
    attempt++
    try {
      ...
      if (!shouldRetryRegistration(error, attempt)) throw error
...
function environmentCredentials(): FeishuCredentials | undefined {
  const appId = process.env.FEISHU_APP_ID?.trim()
  const appSecret = process.env.FEISHU_APP_SECRET?.trim()
  if (appId === undefined || appId.length === 0 || appSecret === undefined || appSecret.length === 0) return undefined
