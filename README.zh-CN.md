...
收到指令后，机器人会立即发送“思考中…”占位消息；Harness 完成后会把该消息更新为回答。回答超过 3500 个 Unicode 字符时自动拆分为多条消息，优先在换行处切分，不会切断 emoji 等 Unicode 字符。飞书 WebSocket 启用 SDK 自动重连和心跳检测，短暂断线或空闲半开连接会自动恢复。

当当前对话中的工具需要审批时，机器人会主动发送工具名、原因和参数摘要。仅发起该 Harness 对话的飞书会话可以回复 `/approve`（只批准本次）或 `/reject`；发送失败、超时和没有对应飞书会话时均拒绝或交给其他已配置的审批渠道处理。

项目和对话选择按飞书 `chat_id` 隔离，并持久化到 `$DSH_HOME/feishu-bot/state.json`。每个对话完成后，机器人会向发起 chat 发送完成通知；配置 `FEISHU_NOTIFY_CHATS` 后也会广播给指定 chat。正在处理消息的 chat 不会收到其他对话的完成通知，避免打断当前聊天；通知中回复 `/uc` 可直接进入最近完成的对话。

## 处理中指示（表情回复）

机器人处理消息时不再发送"思考中…"占位文本，而是在**触发消息**上加 🤔（THINKING）表情；完成后移除 🤔 并保留 ✅（DONE）。最终回答以单条 `对话已完成：…` 消息发回（含完整回复），不再产生占位更新造成的重复内容。

- 需要在飞书开放平台给应用开通**应用身份**权限 `im:message.reactions:write_only`（用户身份版本对 tenant token 无效），并**发布新版本**使权限生效；
- 表情操作为尽力而为：权限缺失或表情不被支持只记日志，不影响正常收发与回答。
